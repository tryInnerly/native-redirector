...
| `shortcuts://x-callback-url/...` | Вызов несуществующего шортката с `x-error=<URL>` — ожидается редирект в браузер по ошибке. На практике iOS открывает приложение «Быстрые команды», показывает ошибку «Файл не существует» и остаётся внутри Shortcuts, не выполняя x-error callback. Проверено февраль 2026, iOS 18.x |
